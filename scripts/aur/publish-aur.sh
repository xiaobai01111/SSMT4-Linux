...
AUR_DIR="$(resolve_aur_source_dir "$SCRIPT_DIR" "$ROOT_DIR")"
...
if grep -Eq "git\\+" "$PKGBUILD_FILE"; then
  echo "==> 检测到 VCS 源码包，保持 SHA256=SKIP"
...
echo "==> 变更预览"
if [[ -z "$(git -C "$AUR_REPO_DIR" status --porcelain -- PKGBUILD .SRCINFO)" ]]; then
  echo "    无变更，跳过提交。"
  exit 0
fi
