...
# Some distros ship pkg-config gdk-pixbuf paths that do not actually exist.
# Fallback to common multiarch locations and continue gracefully if missing.
if [ ! -d "$gdk_pixbuf_binarydir" ]; then
    for candidate in \
        "/usr/lib/$(uname -m)-linux-gnu/gdk-pixbuf-2.0/2.10.0" \
        "/usr/lib64/gdk-pixbuf-2.0/2.10.0"; do
        if [ -d "$candidate" ]; then
            echo "WARNING: gdk-pixbuf binary dir '$gdk_pixbuf_binarydir' not found, using '$candidate'"
            gdk_pixbuf_binarydir="$candidate"
            gdk_pixbuf_moduledir="$candidate/loaders"
            gdk_pixbuf_cache_file="$candidate/loaders.cache"
            break
        fi
    done
fi
...
if [ -d "$gdk_pixbuf_binarydir" ]; then
    ...
    echo "WARNING: gdk-pixbuf binary directory not found, skipping copy"
fi
...
for (( i=0; i<${#FIND_ARRAY[@]}; i+=2 )); do
    ...
    while IFS= read -r -d '' file; do
        ...
        find "$directory" \
            \( -path "$directory/vmware" -o -path "$directory/vmware/*" \) -prune -o \
...
for directory in "${PATCH_ARRAY[@]}"; do
    while IFS= read -r -d '' file; do
        ln -sf $verbose "${file/\/usr\/lib\//}" "$APPDIR/usr/lib" || true
