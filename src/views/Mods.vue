...
const resolveImporter = (gameName: string, configured: unknown) => {
  const mapped = trimValue(gameToImporterMap[gameName]);
  if (mapped) return mapped;
  const normalized = trimValue(configured).toUpperCase();
  return normalized || 'WWMI';
};
...
const buildSummaryFolders = (gameName: string, config: GameConfig) => {
  const migoto = (config.other?.migoto || {}) as Record<string, unknown>;
  const importer = resolveImporter(gameName, migoto.importer);
  const baseMigotoPath = trimValue(migoto.migoto_path) || joinPathString(appDataDir.value, '3Dmigoto-data');
  const explicitImporterFolder = trimValue(migoto.importer_folder);
  const importerFolder =
    explicitImporterFolder
    || (getPathBasename(baseMigotoPath).toUpperCase() === importer ? baseMigotoPath : joinPathString(baseMigotoPath, importer));

  return {
    importer,
    migotoEnabled: Boolean(migoto.enabled),
    modFolder: trimValue(migoto.mod_folder) || joinPathString(importerFolder, 'Mods'),
    shaderFixesFolder: trimValue(migoto.shader_fixes_folder) || joinPathString(importerFolder, 'ShaderFixes'),
  };
};
...
const buildSummary = async (game: (typeof gamesList)[number]): Promise<ModGameSummary> => {
  try {
    ...
    const folders = buildSummaryFolders(game.name, config);
    ...
    return {
      ...
      searchNames: [],
      iconPath: game.iconPath || '',
      importer: resolveImporter(game.name, ''),
      migotoEnabled: false,
      modFolder: '',
      shaderFixesFolder: '',
