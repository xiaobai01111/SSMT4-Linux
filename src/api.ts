...
import type {
  AppSettings,
  ChannelProtectionStatus,
  DialogKind,
  GameConfig,
  GameInfo,
  GameProtectionApplyResult,
  GameProtectionInfo,
  GameProtectionStatus,
  LauncherInfo,
  TelemetryFilesResult,
  TelemetryMutationResult,
  TelemetryStatus,
} from './types/ipc';
import type { AppEventMap, GameDownloadProgressEvent } from './types/events';
export type {
  AppSettings,
...
export type {
  ...
  GameAnticheatWarningEvent,
  GameDownloadOperation,
  GameDownloadProgressEvent,
  GameLifecycleEvent,
} from './types/events';
...
const _ipcCache = new Map<string, CacheEntry<unknown>>();
...
export type DownloadProgress = GameDownloadProgressEvent;
...
export async function checkTelemetryStatus(
  gamePreset: string,
  gamePath?: string,
): Promise<TelemetryStatus> {
  return invoke<TelemetryStatus>('check_telemetry_status', {
    gamePreset,
    gamePath: gamePath || null,
  });
}

export async function checkGameProtectionStatus(
  gamePreset: string,
  gamePath?: string,
): Promise<GameProtectionStatus> {
  return invoke<GameProtectionStatus>('check_game_protection_status', {
    gamePreset,
    gamePath: gamePath || null,
  });
}
...
export async function disableTelemetry(
  gamePreset: string,
  gamePath?: string,
): Promise<TelemetryMutationResult> {
  return invoke<TelemetryMutationResult>('disable_telemetry', {
    gamePreset,
    gamePath: gamePath || null,
  });
}

export async function restoreTelemetry(
  gamePreset: string,
  gamePath?: string,
): Promise<TelemetryMutationResult> {
  return invoke<TelemetryMutationResult>('restore_telemetry', {
    gamePreset,
    gamePath: gamePath || null,
  });
}

export async function removeTelemetryFiles(
  gamePreset: string,
  gamePath: string,
): Promise<TelemetryFilesResult> {
  return invoke<TelemetryFilesResult>('remove_telemetry_files', {
    gamePreset,
    gamePath,
  });
}

export async function applyGameProtection(
  gamePreset: string,
  gamePath: string,
): Promise<GameProtectionApplyResult> {
  return invoke<GameProtectionApplyResult>('apply_game_protection', {
    gamePreset,
    gamePath,
  });
}

export async function getGameProtectionInfo(
  gamePreset: string,
): Promise<GameProtectionInfo> {
  return invoke<GameProtectionInfo>('get_game_protection_info', { gamePreset });
}
...
export async function getLauncherInfo(
  launcherApi: string,
): Promise<LauncherInfo> {
  return invoke<LauncherInfo>('get_launcher_info', { launcherApi });
}
...
export async function updateGamePatch(
  ...
  return invoke('update_game_patch', { launcherApi, gameFolder, taskId });
}
...
export async function verifyGameFiles(
  ...
  return invoke<VerifyResult>('verify_game_files', {
    launcherApi,
    gameFolder,
    taskId,
    bizPrefix: bizPrefix || null,
...
export async function cancelDownload(taskId?: string): Promise<void> {
  return invoke('cancel_download', { taskId: taskId || null });
}
...
export async function openFileDialog(options?: {
  ...
  const multipleResult = result as string[] | null;
  if (Array.isArray(multipleResult) && multipleResult.length > 0) return multipleResult[0];
...
export async function showMessage(
  ...
  const kind = options?.kind === 'success' ? 'info' : options?.kind || 'info';
  await message(msg, {
    title: options?.title,
    kind,
  });
...
export async function askConfirm(
  msg: string,
  options?: { title?: string; kind?: DialogKind; okLabel?: string; cancelLabel?: string }
): Promise<boolean> {
  const kind = options?.kind === 'success' ? 'info' : options?.kind || 'info';
  return ask(msg, {
    title: options?.title,
...
export async function listenAppEvent<K extends keyof AppEventMap>(
  event: K,
  callback: (event: TauriEvent<AppEventMap[K]>) => void
): Promise<() => void> {
