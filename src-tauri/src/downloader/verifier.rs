...
pub async fn verify_game_files(
    ...
    if phase1_failed_count > 0 {
        ...
        emit_progress(&app, task_id, GameDownloadOperation::VerifyGame, &progress);
    }
        ...
        let task = tokio::spawn(async move {
            ...
            if count.is_multiple_of(10) || count == total_files {
                ...
                emit_progress(
                    &app_c,
                    &task_id_c,
                    GameDownloadOperation::VerifyGame,
                    &progress,
                );
            }
        });
...
pub async fn repair_game_files(
    ...
    emit_progress(
        &app,
        task_id,
        GameDownloadOperation::RepairGame,
        &init_progress,
    );
    // 周期性上报进度（并发下载场景下统一汇总）
...
mod tests {
    ...
    #[test]
    fn get_mtime_sec_returns_zero_for_missing_file_and_positive_for_existing_file() {
        let root = unique_temp_dir("ssmt4-verifier-mtime");
        std::fs::create_dir_all(&root).unwrap();
        let file = root.join("data.bin");
        std::fs::write(&file, b"payload").unwrap();

        assert!(get_mtime_sec(&file) > 0);
        assert_eq!(get_mtime_sec(&root.join("missing.bin")), 0);

        let _ = std::fs::remove_dir_all(root);
    }

    #[test]
    fn build_resource_url_trims_slashes_without_encoding() {
        let url = build_resource_url(
            "https://cdn.example.com/",
            "/game/resources/",
            "/Client/Content/Paks/main.pak",
        );

        assert_eq!(
            url,
            "https://cdn.example.com/game/resources/Client/Content/Paks/main.pak"
        );
    }

    #[test]
    fn done_counter_guard_increments_on_drop() {
        let counter = Arc::new(AtomicUsize::new(0));
        {
            let _guard = DoneCounterGuard::new(counter.clone());
    ...
    #[tokio::test]
    async fn remove_invalid_paks_keeps_indexed_files_and_deletes_strays() {
        let root = unique_temp_dir("ssmt4-verifier-paks");
        let paks_dir = root.join("Client").join("Content").join("Paks");
        std::fs::create_dir_all(&paks_dir).unwrap();

        let kept = paks_dir.join("pakchunk0-Windows.pak");
        let stray = paks_dir.join("obsolete.pak");
        std::fs::write(&kept, b"valid").unwrap();
        std::fs::write(&stray, b"stale").unwrap();

        let index = ResourceIndex {
            resource: vec![ResourceFile {
                dest: "Client/Content/Paks/pakchunk0-Windows.pak".to_string(),
                md5: "md5".to_string(),
        ...
        remove_invalid_paks(&root, &index).await;
