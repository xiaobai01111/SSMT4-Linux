...
#[cfg(test)]
mod tests {
    use super::download_with_resume;
    use reqwest::Client;
    ...
    fn unique_temp_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir()
            .join("ssmt4-tests")
            .join(format!("fetcher-{label}-{nonce}.bin"))
    }

    #[tokio::test]
    async fn download_with_resume_honors_pre_cancelled_token_before_request() {
        let client = Client::new();
        let path = unique_temp_path("cancelled");
    ...
    #[tokio::test]
    async fn download_with_resume_skips_existing_file_and_reports_size_without_overwrite() {
        let client = Client::new();
        let path = unique_temp_path("skip-existing");
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).expect("create temp dir");
        }
        std::fs::write(&path, b"existing-content").expect("write existing file");

        let progress = Arc::new(Mutex::new(Vec::new()));
        let progress_sink = progress.clone();
        let callback = move |delta: u64| {
            progress_sink.lock().unwrap().push(delta);
        };
        ...
        assert_eq!(*progress.lock().unwrap(), vec![16]);
