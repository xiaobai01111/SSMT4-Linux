...
use crate::utils::game_dirs::{
    find_game_dir_by_logic_name, get_game_config_path, get_game_dir, get_user_games_dir,
    get_writable_game_dir,
};
...
fn normalize_legacy_game_config(mut config: Value) -> Result<Value, String> {
    if !config.is_object() {
        return Err("游戏配置根对象必须是 JSON object".to_string());
    }
...
pub fn load_game_config(app: tauri::AppHandle, game_name: &str) -> Result<Value, String> {
    ...
    if let Some(json_str) = db::get_game_config(&game_name) {
        ...
        return normalize_legacy_game_config(parsed);
    }
    ...
    let val = normalize_legacy_game_config(val)?;
...
pub fn save_game_config(
    ...
    let game_name = canonical_key(game_name);
    let config = normalize_legacy_game_config(config)?;
...
fn parse_v2_config(json_str: &str, game_name: &str) -> Result<GameInfoConfigV2, String> {
    let config = serde_json::from_str::<GameInfoConfigV2>(json_str)
...
pub fn get_game_key_migration_status(
    ...
    let done = db::read_migration_meta(GAME_KEY_MIGRATION_META_KEY)
        .map(|v| v == "true")
        .unwrap_or(false);
...
pub fn execute_game_key_migration(app: tauri::AppHandle) -> Result<GameKeyMigrationResult, String> {
    ...
    if !preview.needed {
        db::write_migration_meta(GAME_KEY_MIGRATION_META_KEY, "true");
        return Ok(GameKeyMigrationResult {
            success: true,
            migrated: false,
...
mod tests {
    ...
    #[test]
    fn normalize_legacy_game_config_rejects_non_object_root() {
        let error = normalize_legacy_game_config(json!("invalid-root"))
            .expect_err("normalization should fail");
        assert!(error.contains("JSON object"));
    }

    #[test]
    fn validate_name_rejects_dot_prefix_and_excessive_length() {
        let dot_prefixed = validate_game_config_name_internal(".hidden", None);
        assert!(!dot_prefixed.valid);
        assert_eq!(dot_prefixed.code, "INVALID_DOT_PREFIX");

        let too_long = validate_game_config_name_internal(&"a".repeat(65), None);
        assert!(!too_long.valid);
        assert_eq!(too_long.code, "TOO_LONG");
    }

    #[test]
    fn normalize_known_identity_fields_trims_known_paths_and_basic_fields() {
        let mut config = json!({
        ...
        assert_eq!(
            config.get("LogicName").and_then(Value::as_str),
            Some("WutheringWaves")
        );
        assert_eq!(
            config.pointer("/basic/gamePreset").and_then(Value::as_str),
            Some("ZenlessZoneZero")
        );
        assert_eq!(
            config.pointer("/meta/gamePreset").and_then(Value::as_str),
            Some("CustomGame")
        );
    }

    #[test]
    fn rewrite_legacy_paths_in_value_updates_nested_games_and_prefix_paths() {
        let mut config = json!({
            "path": "/home/user/Games/OldGame/Config.json",
            "nested": {
                "prefix": r"C:\Users\test\prefixes\OldGame\drive_c",
                "items": [
                    "/data/Games/OldGame/file.bin",
                    "/data/prefixes/OldGame/cache"
                ]
            }
        });

        rewrite_legacy_paths_in_value(
            &mut config,
            &[("OldGame".to_string(), "NewGame".to_string())],
        );

        assert_eq!(
            config.get("path").and_then(Value::as_str),
            Some("/home/user/Games/NewGame/Config.json")
        );
        assert_eq!(
            config.pointer("/nested/prefix").and_then(Value::as_str),
            Some(r"C:\Users\test\prefixes\NewGame\drive_c")
        );
        assert_eq!(
            config.pointer("/nested/items/0").and_then(Value::as_str),
            Some("/data/Games/NewGame/file.bin")
        );
        assert_eq!(
            config.pointer("/nested/items/1").and_then(Value::as_str),
            Some("/data/prefixes/NewGame/cache")
        );
    }
}
