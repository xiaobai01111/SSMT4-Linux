...
fn compose_api_urls(custom_mirror: Option<&str>) -> Vec<String> {
...
fn build_api_urls() -> Vec<String> {
    let mirror = crate::configs::database::read_setting_value(JADEITE_MIRROR_KEY);
    if let Some(custom_mirror) = mirror
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
    {
        info!("[jadeite] 使用自定义镜像: {}", custom_mirror);
    }
    compose_api_urls(mirror.as_deref())
}

fn parse_release_tag_and_download_url(
...
fn resolve_patch_dir_from_config(config: &serde_json::Value) -> Result<PathBuf, String> {
    // 优先：gameFolder 的父目录
    if let Some(game_folder) = config
...
pub async fn install_jadeite(_app: tauri::AppHandle, game_name: String) -> Result<String, String> {
    ...
    let (tag, download_url) = parse_release_tag_and_download_url(&resp)?;
...
pub fn resolve_patch_dir(game_name: &str) -> Result<PathBuf, String> {
    ...
    resolve_patch_dir_from_config(&data)
}
...
mod tests {
    ...
    #[test]
    fn compose_api_urls_uses_default_when_mirror_missing() {
        let urls = compose_api_urls(None);
        assert_eq!(
            urls,
            vec!["https://codeberg.org/api/v1/repos/mkrsym1/jadeite/releases/latest".to_string()]
        );
    }

    #[test]
    fn compose_api_urls_prioritizes_mirror_and_deduplicates() {
        let mirror = " https://mirror.example.com/jadeite/latest ";
        let urls = compose_api_urls(Some(mirror));
        assert_eq!(
            urls,
            vec![
                "https://mirror.example.com/jadeite/latest".to_string(),
                "https://codeberg.org/api/v1/repos/mkrsym1/jadeite/releases/latest".to_string(),
            ]
        );
    }

    #[test]
    fn compose_api_urls_ignores_blank_mirror() {
        let urls = compose_api_urls(Some("   "));
        assert_eq!(
            urls,
            vec!["https://codeberg.org/api/v1/repos/mkrsym1/jadeite/releases/latest".to_string()]
        );
    ...
    #[test]
    fn parse_release_tag_and_download_url_supports_first_valid_asset() {
        let release = json!({
            "tag_name": "v1.2.3",
            "assets": [
                { "name": "broken-asset" },
                { "browser_download_url": "https://example.com/jadeite.zip" }
            ]
        });

        let (tag, url) = parse_release_tag_and_download_url(&release).expect("should parse");
        assert_eq!(tag, "v1.2.3");
        assert_eq!(url, "https://example.com/jadeite.zip");
    }

    #[test]
    fn parse_release_tag_and_download_url_requires_tag_name() {
        let release = json!({
            "assets": [{ "browser_download_url": "https://example.com/jadeite.zip" }]
        });

        let err = parse_release_tag_and_download_url(&release).expect_err("should fail");
        assert!(err.contains("版本号"));
    }

    #[test]
    fn parse_release_tag_and_download_url_requires_download_url() {
        let release = json!({
            "tag_name": "v1.2.3",
            "assets": [{ "name": "jadeite.zip" }]
        });

        let err = parse_release_tag_and_download_url(&release).expect_err("should fail");
        assert!(err.contains("下载地址"));
    }

    #[test]
    fn resolve_patch_dir_from_config_prefers_game_folder() {
        let config = json!({
            "other": {
                "gameFolder": "/games/zzz/Game",
                "gamePath": "/games/fallback/Game.exe"
            }
        });

        let patch_dir = resolve_patch_dir_from_config(&config).expect("should resolve");
        assert_eq!(patch_dir, PathBuf::from("/games/zzz/patch"));
    }

    #[test]
    fn resolve_patch_dir_from_config_falls_back_to_game_path() {
        let config = json!({
            "other": {
                "gamePath": "/games/sr/Game/Game.exe"
            }
        });

        let patch_dir = resolve_patch_dir_from_config(&config).expect("should resolve");
        assert_eq!(patch_dir, PathBuf::from("/games/sr/patch"));
    }

    #[test]
    fn resolve_patch_dir_from_config_returns_error_without_paths() {
        let config = json!({
            "other": {}
        });

        let err = resolve_patch_dir_from_config(&config).expect_err("should fail");
        assert!(err.contains("无法确定 patch 目录"));
    }
}
