...
use self::hosts::{
    build_managed_telemetry_hosts_block, overwrite_hosts_file_with_pkexec,
    remove_managed_telemetry_hosts_entries,
};
...
#[derive(Debug, Clone)]
struct ChannelProtectionRollback {
    config_path: PathBuf,
    channel_key: String,
    original_value: i64,
    restore_mode: String,
...
impl StagedTelemetryFiles {
    fn stage(game_preset: &str, game_root: &Path) -> Result<Self, String> {
        let dlls = get_telemetry_dlls(game_preset);
        if dlls.is_empty() {
        ...
        for dll_path in &dlls {
            ...
            let original_path = safe_join(game_root, normalized)
                .map_err(|err| format!("遥测文件路径非法 {}: {}", dll_path, err))?;
            ...
            let backup_path = safe_join(&backup_root, normalized)
                .map_err(|err| format!("遥测备份路径非法 {}: {}", dll_path, err))?;
            if let Some(parent) = backup_path.parent() {
                std::fs::create_dir_all(parent)
                    .map_err(|e| format!("创建遥测备份目录失败 {}: {}", parent.display(), e))?;
            }

            if let Err(err) = std::fs::rename(&original_path, &backup_path) {
                let rollback_err = staged.rollback().err();
                ...
                if let Some(rollback_err) = rollback_err {
                    message.push_str(&format!("；回滚失败: {}", rollback_err));
                }
                return Err(message);
            }
    ...
    fn cleanup_backup_root(&mut self) -> Result<(), String> {
        ...
        std::fs::remove_dir_all(root)
            .map_err(|e| format!("清理遥测备份目录失败 {}: {}", root.display(), e))?;
        self.backup_root = None;
        Ok(())
    }
    ...
    fn rollback(&mut self) -> Result<(), String> {
        ...
        for entry in self.staged.iter().rev() {
            ...
            if let Some(parent) = entry.original_path.parent() {
                if let Err(err) = std::fs::create_dir_all(parent) {
                    errors.push(format!("创建回滚目录失败 {}: {}", parent.display(), err));
                    continue;
                }
            }

            if let Err(err) = std::fs::rename(&entry.backup_path, &entry.original_path) {
                errors.push(format!(
                    "恢复遥测文件失败 {} -> {}: {}",
        ...
        if let Err(err) = self.cleanup_backup_root() {
            errors.push(err);
        }
...
fn prepare_channel_protection_rollback(
    game_preset: &str,
    game_root: &Path,
    requested_mode: &str,
    ...
    let restore_mode = db::read_setting_value(&channel_mode_setting_key(game_preset))
        .and_then(|v| normalize_channel_mode(&v, channel.init_value.is_some()))
...
impl ChannelProtectionRollback {
    fn rollback(&self, game_preset: &str) -> Result<(), String> {
...
pub async fn disable_telemetry(
    ...
    let telemetry_result = if servers.is_empty() {
        ...
        if to_block.is_empty() {
            ...
            let append_lines = build_managed_telemetry_hosts_block(&game_preset, &to_block);
...
pub async fn restore_telemetry(
    ...
    let telemetry_result = if servers.is_empty() {
        ...
        if removed_count == 0 {
            ...
            if let Err(err) = overwrite_hosts_file_with_pkexec(&new_content).await {
                error!("[遥测] {}", err);
                return Err(err);
            }
...
pub async fn apply_game_protection(
    ...
    let channel_plan: Option<(String, ChannelProtectionRollback)> =
        if let Some(channel_cfg) = get_channel_protection_config(&game_preset) {
            ...
            let preferred_mode = db::read_setting_value(&mode_key)
    ...
    let telemetry_result =
        match disable_telemetry(game_preset.clone(), Some(game_path.clone())).await {
            Ok(result) => result,
            Err(err) => {
                if let Err(rollback_err) = staged_files.rollback() {
                    return Err(format!(
                        "应用遥测域名屏蔽失败: {}；DLL 回滚失败: {}",
                        err, rollback_err
                    ));
    ...
    let dll_result = serde_json::json!({
        "supported": true,
        "removed": staged_files.removed(),
        "notFound": staged_files.not_found.clone()
    });
        ...
        let channel_state =
            match set_channel_mode_internal(&game_preset, &game_root, &preferred_mode) {
                ...
                Err(err) => {
                    ...
                    if let Err(channel_err) = rollback.rollback(&game_preset) {
                        rollback_errors.push(format!("恢复渠道配置失败: {}", channel_err));
                    }
                    if let Err(restore_err) =
                        restore_telemetry(game_preset.clone(), Some(game_path.clone())).await
                    {
                        rollback_errors.push(format!("恢复 hosts 失败: {}", restore_err));
                    }
                    if let Err(file_err) = staged_files.rollback() {
                        rollback_errors.push(format!("恢复遥测 DLL 失败: {}", file_err));
                    }

                    if rollback_errors.is_empty() {
                        return Err(err);
                    }
                    return Err(format!("{}；回滚失败: {}", err, rollback_errors.join("；")));
                }
            };
    ...
    if let Err(cleanup_err) = staged_files.commit() {
        warn!("[防护] 清理遥测备份目录失败: {}", cleanup_err);
        results.insert("cleanupWarning".to_string(), serde_json::json!(cleanup_err));
    }
...
mod tests {
    ...
    fn sample_telemetry_dll_fixture() -> (String, Vec<String>) {
        for preset in [
            ...
            let dlls = get_telemetry_dlls(preset);
            if !dlls.is_empty() {
