...
pub fn scan_wine_versions(
    ...
    let custom_paths =
        runtime_config::state_view(&settings, |cfg| cfg.custom_search_paths.clone())?;
...
pub fn get_recent_logs(lines: Option<usize>) -> Result<Vec<String>, String> {
    ...
    read_recent_log_lines(&log_dir, max_lines)
}
...
pub async fn fetch_remote_proton_grouped(
    ...
    let (custom_paths, github_token) = runtime_config::state_view(&settings, |cfg| {
        (cfg.custom_search_paths.clone(), cfg.github_token.clone())
    })?;
...
#[cfg(test)]
mod tests {
    use super::read_recent_log_lines;
    ...
    fn unique_temp_dir(label: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir()
            .join("ssmt4-tests")
            .join(format!("wine-manager-{label}-{nonce}"))
    }

    #[test]
    fn get_recent_logs_returns_latest_log_tail_and_handles_missing_dir() {
        let root = unique_temp_dir("recent-logs");
        assert_eq!(
            read_recent_log_lines(&root, 10).expect("missing log dir"),
            Vec::<String>::new()
        );

        std::fs::create_dir_all(&root).expect("create log dir");
        let older = root.join("older.log");
        let newer = root.join("newer.log");
        std::fs::write(&older, "a\nb\nc\n").expect("write older log");
        std::thread::sleep(Duration::from_millis(5));
        std::fs::write(&newer, "1\n2\n3\n4\n").expect("write newer log");

        let lines = read_recent_log_lines(&root, 2).expect("read recent logs");
        assert_eq!(lines, vec!["3".to_string(), "4".to_string()]);
