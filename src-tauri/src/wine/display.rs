...
fn detect_display_server_from_inputs(
    session_type: Option<&str>,
    has_wayland_display: bool,
    has_x_display: bool,
) -> DisplayServer {
    match session_type.unwrap_or_default() {
        "wayland" => {
            if has_x_display {
...
fn detect_display_server() -> DisplayServer {
    ...
    detect_display_server_from_inputs(
...
fn detect_wayland_compositor_from_inputs(
    current_desktop: Option<&str>,
    desktop_session: Option<&str>,
) -> Option<String> {
    current_desktop
        .filter(|value| !value.is_empty())
        .or(desktop_session.filter(|value| !value.is_empty()))
        .map(|value| value.to_string())
}

fn detect_wayland_compositor() -> Option<String> {
    detect_wayland_compositor_from_inputs(
...
mod tests {
    use super::{
        classify_gpu_driver, detect_display_server_from_inputs,
        detect_wayland_compositor_from_inputs, DisplayServer,
    };

    #[test]
    fn detect_display_server_distinguishes_x11_wayland_and_xwayland() {
        assert!(matches!(
            detect_display_server_from_inputs(Some("wayland"), true, true),
            DisplayServer::XWayland
        ));
        assert!(matches!(
            detect_display_server_from_inputs(Some("x11"), false, true),
            DisplayServer::X11
        ));
        assert!(matches!(
            detect_display_server_from_inputs(None, true, false),
            DisplayServer::Wayland
        ));
        assert!(matches!(
            detect_display_server_from_inputs(None, false, false),
            DisplayServer::Unknown
        ));
    }

    #[test]
    fn detect_wayland_compositor_prefers_current_desktop_then_session() {
        assert_eq!(
            detect_wayland_compositor_from_inputs(Some("KDE"), Some("plasma")),
            Some("KDE".to_string())
        );
        assert_eq!(
            detect_wayland_compositor_from_inputs(None, Some("hyprland")),
            Some("hyprland".to_string())
        );
    }

    #[test]
    fn classify_gpu_driver_handles_known_vendor_names() {
        assert_eq!(
            classify_gpu_driver("NVIDIA GeForce RTX 4090"),
            "nvidia".to_string()
        );
        assert_eq!(
            classify_gpu_driver("AMD Radeon RX 7900 XTX"),
            "amdgpu".to_string()
        );
        assert_eq!(
            classify_gpu_driver("Intel Arc Graphics"),
            "intel".to_string()
        );
        assert_eq!(classify_gpu_driver("Virtual GPU"), "unknown".to_string());
    }
}
