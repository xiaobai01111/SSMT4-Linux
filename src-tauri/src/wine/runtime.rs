...
#[cfg(test)]
mod tests {
    use super::list_available_runtimes;
    use std::collections::HashSet;

    #[test]
    fn list_available_runtimes_exposes_unique_components_and_expected_families() {
        let runtimes = list_available_runtimes();
        let ids = runtimes
            .iter()
            .map(|component| component.id.as_str())
            .collect::<HashSet<_>>();
        let categories = runtimes
            .iter()
            .map(|component| component.category.as_str())
            .collect::<HashSet<_>>();

        assert_eq!(ids.len(), runtimes.len());
        assert!(ids.contains("vcrun2015"));
        assert!(ids.contains("dotnet48"));
        assert!(ids.contains("d3dx9"));
        assert!(ids.contains("mf"));
        assert_eq!(
            categories,
            HashSet::from(["vcrun", "dotnet", "directx", "fonts", "media"])
        );
    }
}
