...
pub fn ensure_parent_dir(path: &Path, action: &str) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        ensure_dir(parent).map_err(|e| {
            format!(
                "{action}: failed to ensure parent {}: {e}",
                parent.display()
            )
        })?;
    }
...
pub fn remove_dir_if_exists(path: &Path, action: &str) -> Result<(), String> {
    if path.exists() {
        std::fs::remove_dir_all(path)
            .map_err(|e| format!("{action}: failed to remove {}: {}", path.display(), e))?;
    }
    Ok(())
}

pub fn rename_path(from: &Path, to: &Path, action: &str) -> Result<(), String> {
    ensure_parent_dir(to, action)?;
...
fn default_xdg_data_dir_from_env(xdg_data_home: Option<&str>, home: Option<&str>) -> PathBuf {
...
mod tests {
    ...
    fn unique_temp_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir()
            .join("ssmt4-tests")
            .join(format!("file-manager-{label}-{nonce}"))
    }
    ...
    #[test]
    fn get_default_xdg_data_dir_prefers_xdg_then_home_then_tmp() {
        assert_eq!(
            default_xdg_data_dir_from_env(Some("/tmp/xdg-data"), Some("/tmp/home-a")),
            PathBuf::from("/tmp/xdg-data").join("ssmt4")
        );

        assert_eq!(
            default_xdg_data_dir_from_env(None, Some("/tmp/home-b")),
            PathBuf::from("/tmp/home-b/.local/share/ssmt4")
        );

        assert_eq!(
            default_xdg_data_dir_from_env(None, None),
            PathBuf::from("/tmp/ssmt4/data")
        );
    }

    #[test]
    fn ensure_dir_and_copy_dir_recursive_create_and_copy_nested_content() {
        let src = unique_temp_dir("copy-src");
        let dst = unique_temp_dir("copy-dst");
        std::fs::create_dir_all(src.join("nested")).expect("create src dir");
        std::fs::write(src.join("root.txt"), "root").expect("write root file");
        std::fs::write(src.join("nested").join("child.txt"), "child").expect("write child file");

        ensure_dir(&dst).expect("ensure dst dir");
        copy_dir_recursive(&src, &dst).expect("copy dir recursively");
    ...
    #[test]
    fn ensure_parent_dir_write_and_rename_helpers_manage_side_effects() {
        let root = unique_temp_dir("managed-side-effects");
        let src = root.join("from").join("state.txt");
        let dst = root.join("to").join("state.txt");

        ensure_parent_dir(&src, "write managed file").expect("ensure write parent");
        std::fs::write(&src, "hello").expect("write managed file");
        assert_eq!(std::fs::read_to_string(&src).unwrap(), "hello");

        rename_path(&src, &dst, "rename managed file").expect("rename managed file");
        assert!(!src.exists());
    ...
    fn remove_dir_if_exists_is_idempotent() {
        let root = unique_temp_dir("remove-dir");
        std::fs::create_dir_all(root.join("nested")).expect("create nested dir");

        remove_dir_if_exists(&root, "remove managed dir").expect("remove existing dir");
        assert!(!root.exists());
    ...
    #[test]
    fn migrate_dir_contents_preserves_existing_target_files() {
        let src = unique_temp_dir("migrate-src");
        let dst = unique_temp_dir("migrate-dst");
        std::fs::create_dir_all(src.join("nested")).expect("create src dir");
        std::fs::create_dir_all(&dst).expect("create dst dir");
        std::fs::write(src.join("nested").join("new.txt"), "from-src").expect("write new file");
        std::fs::write(src.join("keep.txt"), "src-version").expect("write src keep");
        std::fs::write(dst.join("keep.txt"), "dst-version").expect("write dst keep");

        migrate_dir_contents(&src, &dst).expect("migrate dir contents");

        assert_eq!(
            std::fs::read_to_string(dst.join("nested").join("new.txt")).unwrap(),
            "from-src"
        );
        assert_eq!(
