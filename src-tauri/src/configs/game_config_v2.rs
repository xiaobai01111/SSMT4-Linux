...
impl GameInfoConfigV2 {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.schema_version != GAME_INFO_SCHEMA_VERSION {
            return Err(format!(
                "unsupported game info schema version: {}",
                self.schema_version
            ));
        }
        if self.game_name.trim().is_empty() {
            return Err("gameName must not be empty".to_string());
        }
        if self.meta.game_preset.trim().is_empty() {
            return Err("meta.gamePreset must not be empty".to_string());
        }
        if self.meta.display_name.trim().is_empty() {
            return Err("meta.displayName must not be empty".to_string());
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn normalized_preserves_explicit_schema_version_for_validation() {
        let config = GameInfoConfigV2 {
            schema_version: 99,
            ..GameInfoConfigV2::new("WutheringWaves")
        }
        .normalized("WutheringWaves");
    ...
    fn validate_rejects_unsupported_schema_version() {
        ...
        assert!(error.contains("unsupported game info schema version"));
    }
    ...
    fn validate_rejects_missing_required_fields() {
        ...
        let error = config.validate().expect_err("validation should fail");
        assert!(error.contains("gameName"));
    }
}
