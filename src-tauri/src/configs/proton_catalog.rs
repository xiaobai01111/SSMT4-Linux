...
mod tests {
    ...
    #[test]
    fn catalog_helpers_validate_keys_urls_and_patterns() {
        assert!(is_valid_family_key("ge-proton_10"));
        assert!(!is_valid_family_key("ge proton"));

        assert!(is_valid_repo("GloriousEggroll/proton-ge-custom"));
        assert!(!is_valid_repo("invalid_repo"));

        assert!(is_valid_url("https://example.com/releases"));
        assert!(!is_valid_url("ftp://example.com/releases"));

        assert!(matches_pattern("GE-Proton10-30", "(?i)ge-proton"));
        assert!(matches_pattern("wine-ge-custom", "(?i)GE-CUSTOM"));
        assert!(!matches_pattern("wine-ge-custom", ""));
    }

    #[test]
    fn proton_asset_name_normalization_and_arch_detection_are_stable() {
        assert_eq!(
            normalize_asset_name_tokens("GE-Proton_10.0-rc1+x86_64.tar.gz"),
            " ge proton 10 0 rc1 x86 64 tar gz "
        );
        assert!(is_explicit_x86_64_asset("wine-amd64.tar.xz"));
        assert!(!is_explicit_x86_64_asset("wine-arm64.tar.xz"));
    }

    #[test]
    fn release_api_candidates_and_query_helpers_build_expected_urls() {
        let source = ProtonSource {
            id: None,
            family_key: "ge-proton".to_string(),
            provider: "github_releases".to_string(),
            repo: "GloriousEggroll/proton-ge-custom".to_string(),
            endpoint: String::new(),
            url_template: String::new(),
            asset_index: -1,
            asset_pattern: "(?i)\\.tar\\.(gz|xz)$".to_string(),
        ...
        let urls = release_api_candidates(&source, 20);
        assert_eq!(
            urls[0],
        ...
        assert_eq!(
            urls[1],
            "https://ghp.ci/api.github.com/repos/GloriousEggroll/proton-ge-custom/releases?per_page=20"
        );

        assert_eq!(
            add_or_replace_query_param("https://example.com/api", "page", "2"),
            "https://example.com/api?page=2"
        );
        assert_eq!(
            add_or_replace_query_param("https://example.com/api?per_page=10", "page", "2"),
            "https://example.com/api?per_page=10&page=2"
        );
    }

    #[test]
    fn github_token_and_failure_summary_helpers_match_expected_hosts() {
        ...
        assert_eq!(
            summarize_source_failure("请求 GE-Proton 失败: HTTP 403"),
            "GE-Proton (请求受限)"
        );
        assert_eq!(
            summarize_source_failure("请求 GE-Proton 失败: failed to lookup address information"),
            "GE-Proton (DNS 解析失败)"
        );
        assert_eq!(summarize_source_failure("plain failure"), "plain failure");
    }
}
